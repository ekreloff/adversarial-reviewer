...
description: "Adversarial code review that breaks the self-review monoculture. Use when reviewing code changes, before merging a PR, or when you suspect the AI is being too agreeable about code quality. Forces perspective shifts through three hostile reviewer personas that catch blind spots."
...
When an AI reviews code it wrote (or just read), it shares the same mental model and blind spots as the author. This produces "LGTM" reviews on code a fresh human reviewer would flag immediately.

This skill forces a genuine perspective shift by requiring three adversarial personas — each with different priorities, different fears, and different definitions of "bad code."
...
| Cosmetic-only findings | Reporting only whitespace/formatting while missing a null dereference is worse than no review. Substance first, style second. |
...
- **When the AI said "looks good"** — if you got an easy approval, run this for a second opinion
